 ...
 */
  ...
  if(isset($row->field_field_photo_copy[0]['rendered']['#markup'])) {
    $field_body =  truncate_utf8($row->field_field_photo_copy[0]['rendered']['#markup'], 140, $wordsafe = TRUE);
  }
  ...
  if(isset($row->field_field_video[0]))  {
    ...
    elseif ($file_type == 'youtube') {
      ...
      $field_video = theme('media_' . $file_type . '_video', array('uri' => $row->field_field_video[0]['raw']['uri'], 'width' => '640', 'height' => '480', 'autoplay' => 1));
    }
  }
